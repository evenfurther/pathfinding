...
mod ex1 {
    ...
    fn dijkstra_ok() {
        for target in 0..9 {
            assert_eq!(dijkstra(&1, neighbours, |&node| node == target),
                       expected(target));
        }
    }
    ...
    fn fringe_ok() {
        for target in 0..9 {
            assert_eq!(fringe(&1, neighbours, |_| 0, |&node| node == target),
                       expected(target));
        }
    }
...
";
    ...
    fn astar_no_path() {
        ...
        assert_eq!(astar(&(2, 3), neighbours, |n| distance(n, &GOAL), |n| n == &GOAL),
                   None);
    }
    ...
    fn fringe_no_path() {
        const GOAL: (usize, usize) = (1, 1);
        assert_eq!(fringe(&(2, 3), neighbours, |n| distance(n, &GOAL), |n| n == &GOAL),
                   None);
    }
